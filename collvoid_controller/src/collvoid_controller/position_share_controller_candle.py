...
class PositionShareController(object):
    def __init__(self):
        # Set the name
        self.name = "999"
        self.name = rospy.get_param('/robot_attribute/number', self.name)
        ...
        rospy.Subscriber('/position_share_me', PoseTwistWithCovariance, self.position_share_me_cb, queue_size=1)
        rospy.Subscriber('/position_share_neighbors', PoseTwistWithCovariance, self.position_share_neighbors_cb, queue_size=1)
        rospy.Service('get_neighbors', GetNeighbors, self.get_neighbors_cb)

    def position_share_me_cb(self, msg):
        """
        :param msg:
    ...
    def position_share_neighbors_cb(self, msg):
        with self.neighbors_lock:
            if msg.robot_id == self.name:
                return
            if msg.robot_id not in self.neighbors:
            ...
            robot['holo_speed'] = msg.holonomic_velocity
            robot['radius'] = msg.radius
            robot['last_seen'] = msg.header.stamp
